...
async function parseDevicesSwift() {
  ...
  let webcams = [];
  // last line is mic stuff
  let mics = JSON.parse(output[output.length - 1]);
  for (let i = 0; i < output.length - 1; i++) {
    let obj = JSON.parse(output[i]);
    
    // converting to same format as windows parsing
    webcams.push({
      name: obj.name,
      framerate: parseInt(obj.framerate),
      resolution: obj.res.width + 'x' + obj.res.height
    });
  }
