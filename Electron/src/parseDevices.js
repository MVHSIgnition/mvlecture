...
module.exports = async () => {
  const listDevicesCmd = ['-list_devices true', '-f dshow', '-i dummy'];
  let webcams = [];
  let mics = [];

  let { error, stdout, stderr } = await execp(__dirname+'\\ffmpeg.exe',listDevicesCmd);
  ...
  for (let i = 0; i < webcams.length; i++) {
    const listOptionsCmd = [`-list_options true`, `-f dshow`, `-i video="${webcams[i].name}"`];
    let { error, stdout, stderr } = await execp('ffmpeg',listDevicesCmd);
    const output = error.toString();
